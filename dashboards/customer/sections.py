...
def render_main(data, theme):
    classification = load_csv("product_classification.csv")
    review_count = (
        int(classification["대분류"].eq("기타").sum())
        if "대분류" in classification.columns
        else 0
    )
    title_column, refresh_column = st.columns([4, 1], vertical_alignment="center")
    with title_column:
    ...
    with refresh_column:
        refresh_clicked = st.button(
            "🔄 데이터 새로고침",
            key="home_data_refresh",
            width="stretch",
            help="최신 주문 파일을 다시 읽고 품목명을 자동분류합니다.",
        )
        if st.button(
            f"⚠️ 검토 필요 {review_count:,}건 보기",
            key="home_open_product_review",
            width="stretch",
            disabled=review_count == 0,
        ):
            st.query_params.clear()
            st.query_params["dashboard"] = "product"
            st.query_params["review"] = "unclassified"
            st.session_state["product_view"] = "검토 필요"
            st.rerun()

    if refresh_clicked:
        with st.spinner("최신 주문 데이터를 읽고 품목명을 자동분류하는 중입니다..."):
            try:
                ...
                st.error(f"데이터 새로고침 중 오류가 발생했습니다: {error}")

    refresh_result = st.session_state.pop("home_refresh_result", None)
    if refresh_result:
        st.success(
            f"품목 {refresh_result['total']:,}건을 확인했습니다. "
            f"자동분류 {refresh_result['classified']:,}건 · "
            f"검토 필요 {refresh_result['unclassified']:,}건"
        )
