...
def run_validation():
    ...
    # 8. 원본 데이터 금액 일치
    # merged_data 의 유효 주문 금액 합 vs orders.csv 의 유효 금액 합
    ...
    non_member_in_metrics = set(metrics["customer_id"].unique()) - set(members["customer_id"].unique())
    # 비회원 고객이 metrics 에 있으면 "비회원" 표시가 있어야 함
    non_member_metrics = metrics[metrics["customer_id"].isin(non_member_in_merged)]
    ok = len(non_member_metrics) == 0 or all(non_member_metrics["회원 여부"] == "비회원")
    results.append({
        "항목": "비회원 고객 별도 표시",
        "상태": "통과" if ok else "실패",
