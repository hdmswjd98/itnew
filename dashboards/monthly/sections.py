...
from services.monthly.export import create_excel_report, create_pdf_report
from services.monthly.repository import ReportStoreError, get_report_store
from shared.paths import DATA_DIR, INPUT_DIR, OUTPUT_DIR
...
def render(theme):
    ...
    if merged.empty or "order_date_parsed" not in merged:
        st.warning("고객 분석 파이프라인을 먼저 실행해주세요: python3 -m services.customer.pipeline")
        return
    try:
