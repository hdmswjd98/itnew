...
def find_source_excel():
    """프로젝트 루트 및 상위에서 *.xlsx를 찾는다. 없으면 명시적인 오류."""
    candidates = [
        path for path in REPO_ROOT.glob("*.xlsx")
        if not path.name.startswith("~$")
    ]
    # rglob로 상위까지 훑기 (첫 발견 기준)
    if not candidates:
        for parent in [REPO_ROOT] + list(REPO_ROOT.parents)[:4]:
            for path in parent.rglob("*.xlsx"):
                if not path.name.startswith("~$"):
                    candidates.append(path)
            if candidates:
                break
    if not candidates:
        raise FileNotFoundError("[얼른] 분석 대상 Excel 파일을 찾지 못했습니다.")
    return candidates[0]
