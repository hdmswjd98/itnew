    ...
    <div className="min-h-screen bg-[#f6f8fb] text-[#17202a] transition-colors dark:bg-[#111827] dark:text-slate-100 lg:flex">
      ...
      <main className="min-w-0 flex-1 lg:ml-64">
        <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-[#e6eaf0] bg-white/90 px-6 backdrop-blur transition-colors dark:border-slate-700 dark:bg-slate-900/90 lg:px-10">
          ...
          <div className="flex items-center gap-4"><div className="hidden text-xs text-slate-500 dark:text-slate-400 sm:block">{data ? `마지막 갱신 ${new Date(data.updatedAt).toLocaleString("ko-KR")}` : "데이터 확인 중"}</div><button onClick={toggleTheme} aria-label="테마 전환" className="grid h-9 w-9 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600 transition hover:border-[#FA9C00] dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">{theme === "light" ? <Moon size={17}/> : <Sun size={17}/>}</button></div>
        </header>
        <div className="p-6 lg:p-10">
          {error && <div className="mb-5 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div>}
          {loading && !data ? <Loading /> : view === "home" ? <HomeDashboard data={data} onNavigate={(next) => setView(next)} /> : view === "product" ? <ProductDashboard data={data} productView={productView} setProductView={setProductView} reload={loadData} /> : view === "customer" ? <CustomerDashboard /> : <MonthlyDashboard />}
        </div>
      </main>
    </div>
  );
