...
| 파일 | 설명 |
|---|---|
| `data/output/product_classification.csv` | 품목명 → 카테고리 분류 결과 |
| `data/output/category_summary.csv` | 대분류별 요약 |
| `data/output/subcategory_summary.csv` | 중분류별 요약 |
| `data/output/top_products.csv` | 상위 품목 랭킹 |
| `data/output/product_demand_analysis.csv` | 대분류 × 고객군 수요 분석 |
| `data/output/product_demand_subcategory.csv` | 중분류 × 고객군 수요 분석 |

## 🛠️ 실행 방법

```bash
# 1. 데이터 준비
cp /path/to/orders.csv data/input/orders.csv

# 2. 품목명 분류
python3 -m services.product.classifier

# 3. 수요 분석
python3 -m services.product.analytics
...
`services/product/classifier.py`의 `CATEGORY_RULES` 딕셔너리를 수정하여 분류 규칙을 확장할 수 있습니다:
